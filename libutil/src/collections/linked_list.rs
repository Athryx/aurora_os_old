...
impl<T: ListNode> LinkedList<T>
{
	...
	pub fn push(&mut self, val: MemOwner<T>) -> UniqueMut<T>
	{
		...
		unsafe { UniqueMut::from_ptr(val.ptr_mut()) }
	}
	...
	pub fn insert_before(&mut self, new_node: MemOwner<T>, node: impl UniquePtr<T>)
		-> UniqueMut<T>
	{
		assert!(self.len != 0);
		self.len += 1;
		...
		unsafe { UniqueMut::from_ptr(new_node.ptr_mut()) }
	}
	...
	pub fn remove_node(&mut self, node: impl UniquePtr<T>) -> MemOwner<T>
	{
		...
		unsafe { MemOwner::from_raw(node.ptr() as *mut T) }
	}
