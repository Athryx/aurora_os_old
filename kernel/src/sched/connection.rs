...
impl ConnectionMap
{
	...
	{
		self.data
			.binary_search_by(|probe| probe.this_cpid().pid().cmp(&conn_id))
	}
...
impl Connection
{
	...
	{
		unsafe {
			tlist.lock()[ThreadState::Listening(self.cpids)]
				.get(0)
				.map(|ptr| ptr.unbound())
		}
	}

	pub fn send_message(&self, args: &mut MsgArgs, options: MsgOptions)
		-> Result<Registers, SysErr>
	{
		...
		match self.get_waiting_thread() {
			Some(thread) => {
				...
				Thread::move_to(
					thread,
					ThreadState::Waiting(thread_c().tuid()),
					&mut thread_list,
				);
			},
			None => {
				...
				match handler.options().block_mode {
					BlockMode::NonBlocking => {
						...
						other_process
							.new_thread_regs(regs, Some(format!("domain_handler_{}", self.domain)))
							.or(Err(SysErr::MsgUnreach))?;
						mv.confirm_success();
					},
					BlockMode::Blocking(tid) => match other_process.get_thread(tid) {
						Some(thread) => {
							let mut mv = SMemMover::new(args, options, &process, &other_process);
						...
						None => {
							other_process
								.domains()
								.lock()
								.remove(other_process.pid(), Some(self.domain));
